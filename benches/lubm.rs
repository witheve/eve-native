...
use eve::ops::{Block, Constraint, Program, make_scan, register};
...
fn rand(rseed: &mut u32) -> u32 {
    *rseed = ((Wrapping(*rseed) * Wrapping(1103515245) +
                   Wrapping(12345)) &
                  Wrapping(0x7fffffff))
             .0;
    return *rseed;
...
fn eav(program: &mut Program,
       eavs: &mut Vec<(u32, u32, u32)>,
       e: &str,
       a: &str,
       v: &str) {
    eavs.push((program.state.interner.string_id(&e),
               program.state.interner.string_id(&a),
               program.state.interner.string_id(&v)))
}

fn make_faculty(program: &mut Program,
                eavs: &mut Vec<(u32, u32, u32)>,
    ...
    // every Faculty is teacherOf 1~2 GraduateCourses
    for _ in 0..rand_between(&mut seed, 1, 2) {
        let course = format!("{}|graduate_course{}",
                             department,
                             *grad_course_ix);
...
fn random_professor(seed: &mut u32,
                    department: &str,
                    fulls: u32,
                    associates: u32,
                    assistants: u32)
...
fn generate(program: &mut Program,
            university_count: usize)
    -> Vec<(u32, u32, u32)> {
    let mut counter = 0;
    ...
    for university_ix in 0..university_count {
        ...
        for department_ix in 0..department_count {
            let department =
                format!("{}|department{}", university, department_ix);
            eav(program,
                &mut eavs,
                &department,
                "tag",
            ...
            // one of the FullProfessors is headOf the Department
            let head =
                rand_between(&mut seed, 0, full_professors_count);
            let prof =
                format!("{}|full_professor{}", department, head);
            eav(program, &mut eavs, &prof, "head of", &department);

            // 10~14 AssociateProfessors worksFor the Department
            let associate_professors_count =
                rand_between(&mut seed, 10, 14);
            ...
            // 8~11 AssistantProfessors worksFor the Department
            let assistant_professors_count =
                rand_between(&mut seed, 8, 11);
            ...
            // 10~20 ResearchGroups are subOrgnization of the Department
            let research_group_count =
                rand_between(&mut seed, 10, 20);
            for rg_ix in 0..research_group_count {
                let group =
                    format!("{}|research_group{}", department, rg_ix);
                eav(program,
                    &mut eavs,
                    &group,
                    "tag",
            ...
            // UndergraduateStudent : Faculty = 8~14 : 1
            let undergrads_count = rand_between(&mut seed, 8, 14) *
                total_faculty;
            ...
            for ug_ix in 0..undergrads_count {
                let undergrad =
                    format!("{}|undergrad{}", department, ug_ix);
                eav(program,
                    &mut eavs,
                    &undergrad,
                    "tag",
                ...
                for _ in 0..course_count {
                    let undergrad_course_ix =
                        rand_between(&mut seed, 0, course_ix);
                    eav(program,
                        &mut eavs,
                        &undergrad,
                        "takes-course",
            ...
            let grads_count = rand_between(&mut seed, 3, 4) *
                total_faculty;
            ...
            for g_ix in 0..grads_count {
                ...
                for _ in 0..course_count {
                    let course_ix =
                        rand_between(&mut seed, 0, grad_course_ix);
                    eav(program,
                        &mut eavs,
                        &grad,
                        "takes-course",
                        &format!("{}|graduate_course{}",
                                 department,
                                 course_ix));
                }
                // every GraduateStudent has a Professor as his advisor
                ...
                // every GraudateStudent has an undergraduateDegreeFrom a University
                let degree = rand_between(&mut seed,
                                          0,
                                          university_count as u32);
                eav(program,
                    &mut eavs,
                    &grad,
                    "undergraduate-degree-from",
                ...
                for _ in 0..paper_count {
                    ...
                    let publication =
                        format!("{}|publication{}", prof, pub_ix);
                    eav(program,
                        &mut eavs,
                        &publication,
                        "author",
...
#[bench]
pub fn do_lubm_1(b: &mut Bencher) { do_bench(b, "lubm1", lubm_1); }
#[bench]
pub fn do_lubm_2(b: &mut Bencher) { do_bench(b, "lubm2", lubm_2); }
#[bench]
pub fn do_lubm_3(b: &mut Bencher) { do_bench(b, "lubm3", lubm_3); }
#[bench]
pub fn do_lubm_4(b: &mut Bencher) { do_bench(b, "lubm4", lubm_4); }
#[bench]
pub fn do_lubm_5(b: &mut Bencher) { do_bench(b, "lubm5", lubm_5); }
#[bench]
pub fn do_lubm_7(b: &mut Bencher) { do_bench(b, "lubm7", lubm_7); }
#[bench]
pub fn do_lubm_8(b: &mut Bencher) { do_bench(b, "lubm8", lubm_8); }
#[bench]
pub fn do_lubm_9(b: &mut Bencher) { do_bench(b, "lubm9", lubm_9); }
#[bench]
pub fn do_lubm_11(b: &mut Bencher) { do_bench(b, "lubm11", lubm_11); }
#[bench]
pub fn do_lubm_12(b: &mut Bencher) { do_bench(b, "lubm12", lubm_12); }
#[bench]
pub fn do_lubm_13(b: &mut Bencher) { do_bench(b, "lubm13", lubm_13); }
#[bench]
pub fn do_lubm_14(b: &mut Bencher) { do_bench(b, "lubm14", lubm_14); }
