...
pub type GetIteratorFunc = Fn(&mut EstimateIter,
                              &mut RuntimeState,
                              &mut Frame)
                              -> bool;
...
impl Solver {
    pub fn new(interner: &mut Interner,
        ...
        match active_scan {
            ...
            Some(&Constraint::IntermediateScan {
                     ref full_key, ..
                 }) |
                ...
                for (field_ix, field) in full_key.iter().enumerate() {
                    if let &Field::Register(offset) = field {
                        ...
                        if let &Field::Value(value) = field {
                            intermediate_accepts.push((field_ix,
                                                       value));
                        }
                    }
                }
        ...
        for (ix, constraint) in constraints.iter().enumerate() {
            ...
            match constraint {
                ...
                &Constraint::Function { .. } => {
                    ...
                    accepts.push(make_function_accept(constraint,
                                                      ix));
                }
                ...
                &Constraint::Watch {
                    ...
                    watch_registers.push((name.to_owned(),
                                          registers.clone()));
        ...
        let outputs =
            output_funcs.iter()
                        .map(|&x| match x {
                                 OutputFuncs::Bind => {
                                     do_bind as OutputFunc
                                 }
                                 OutputFuncs::Commit => {
                                     do_commit as OutputFunc
                                 }
                                 OutputFuncs::DynamicCommit => {
                                     do_dynamic_commit as OutputFunc
                                 }
                                 OutputFuncs::Watch => {
                                     do_watch as OutputFunc
                                 }
                                 OutputFuncs::Intermediate => {
                                     do_intermediate_insert as
                                         OutputFunc
                                 }
                                 OutputFuncs::Project => {
                                     do_project as OutputFunc
                                 }
                                 OutputFuncs::Aggregate => {
                                     do_aggregate as OutputFunc
                                 }
                             })
                        .collect();
    ...
    pub fn do_move(&self,
        ...
        if self.moves.len() > 0 {
            let change =
                frame.input
                     .expect("running solver without an input!");
    ...
    pub fn do_intermediate_move(&self, frame: &mut Frame) -> bool {
        if let Some(ref intermediate) = frame.intermediate {
            for &(from, to) in self.moves.iter() {
                frame.row
                     .set_multi(to, intermediate.key[from]);
            }
...
pub fn make_scan_get_iterator(scan: &Constraint,
                              ix: usize)
    -> Arc<GetIteratorFunc> {
...
pub fn make_scan_accept(scan: &Constraint,
                        me: usize)
    -> Arc<AcceptFunc> {
    let (e, a, v, register_mask) = match scan {
        &Constraint::Scan {
    ...
    Arc::new(move |state, frame, cur_constraint| {
        ...
        if cur_constraint == me ||
            !has_any_bits(register_mask, frame.row.solving_for)
        {
            return true;
        }
        let resolved_e = frame.resolve(&e);
        ...
        let resolved_v = frame.resolve(&v);
        state.index
             .check(resolved_e, resolved_a, resolved_v)
    })
}
...
pub fn make_scan_get_rounds(scan: &Constraint) -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        ...
        let resolved_v = frame.resolve(&v);
        let iter =
            state.distinct_index
                 .iter(resolved_e, resolved_a, resolved_v);
        state.output_rounds
             .compute_output_rounds(iter);
    })
}
...
pub fn make_commit_lookup_get_rounds(scan: &Constraint)
    -> Arc<GetRoundsFunc> {
    let (e, a, v, _) = match scan {
        &Constraint::LookupCommit {
            e,
...
pub fn make_lookup_remote_get_iterator(scan: &Constraint,
                                       ix: usize)
    -> Arc<GetIteratorFunc> {
    let (e, a, v, _for, _type, from, to, register_mask) =
        match scan {
            &Constraint::LookupRemote {
                e,
                a,
                ...
                state.remote_index
                    ...
                    (resolved_e == 0 || x.e == resolved_e) &&
                        ...
                        (resolved_for == 0 ||
                             x._for == resolved_for) &&
                        (resolved_type == 0 ||
                             x._type == resolved_type) &&
                        (resolved_from == 0 ||
                             x.from == resolved_from) &&
...
pub fn make_filter_accept(scan: &Constraint,
                          me: usize)
    -> Arc<AcceptFunc> {
    ...
    Arc::new(move |state, frame, cur_constraint| {
        if cur_constraint == me ||
            !has_any_bits(param_mask, frame.row.solving_for)
        {
            return true;
        }
        if check_bits(frame.row.solved_fields, param_mask) {
            let resolved_left =
                state.interner
                     .get_value(frame.resolve(&left));
            let resolved_right =
                state.interner
                     .get_value(frame.resolve(&right));
...
pub fn make_function_get_iterator(scan: &Constraint,
                                  ix: usize)
    -> Arc<GetIteratorFunc> {
    let (func, output, params, param_mask, output_mask) =
        match scan {
            &Constraint::Function {
                ref func,
                ref output,
    ...
    Arc::new(move |iter, state, frame| {
        ...
        if check_bits(solved, param_mask) &&
            !check_bits(solved, output_mask)
        {
            let result = {
            ...
            match result {
                Some(v) => {
                    if iter.is_better(1) {
                        ...
                        let reg =
                            if let Field::Register(reg) = output {
                                reg
                            } else {
...
pub fn make_function_accept(scan: &Constraint,
                            me: usize)
    -> Arc<AcceptFunc> {
    let (func, output, params, param_mask, output_mask) =
        match scan {
    ...
    Arc::new(move |state, frame, cur_constraint| {
        ...
        let solved = frame.row.solved_fields;
        let solving_output_with_inputs = check_bits(solved,
                                                    param_mask) &&
            has_any_bits(frame.row.solving_for, output_mask);
        let solving_input_with_output =
            check_bits(solved, param_mask | output_mask) &&
        ...
        let result = {
            let mut resolved = vec![];
            for param in params.iter() {
                resolved.push(state.interner
                                   .get_value(frame.resolve(param)));
            }
            func(resolved)
        };
        match result {
...
pub fn make_multi_get_iterator(scan: &Constraint,
                               ix: usize)
    -> Arc<GetIteratorFunc> {
    let (func, output_fields, params, param_mask, output_mask) =
        match scan {
            &Constraint::MultiFunction {
                ref func,
...
pub fn make_multi_accept(_: &Constraint,
                         _: usize)
    -> Arc<AcceptFunc> {
    // let (e,a,v,register_mask) = match scan {
...
pub fn make_intermediate_get_iterator(scan: &Constraint,
                                      ix: usize)
    -> Arc<GetIteratorFunc> {
    ...
    Arc::new(move |mut iter, state, frame| {
        ...
        let resolved = key.iter()
                          .map(|param| frame.resolve(param))
                          .collect();
        let outputs =
            value.iter()
                 .map(|x| if let &Field::Register(reg) = x {
                reg
            } else {
                panic!("Non-register intermediate scan output")
            })
                 .collect();
        if state.intermediates
                .propose(&mut iter, resolved, outputs)
        {
            iter.constraint = ix;
        }
        true
...
pub fn make_intermediate_accept(scan: &Constraint,
                                me: usize)
    -> Arc<AcceptFunc> {
    let (key, value, register_mask, output_mask) = match scan {
        &Constraint::IntermediateScan {
    ...
    Arc::new(move |state, frame, cur_constraint| {
        ...
        let resolved = key.iter()
                          .map(|param| frame.resolve(param))
                          .collect();
        let resolved_value = value.iter()
                                  .map(|param| frame.resolve(param))
                                  .collect();

        state.intermediates
             .check(&resolved, &resolved_value)
    })
}

pub fn make_intermediate_get_rounds(scan: &Constraint)
    -> Arc<GetRoundsFunc> {
    ...
    Arc::new(move |state, frame| {
        let resolved: Vec<Interned> =
            key.iter()
               .map(|v| frame.resolve(v))
               .collect();
        let resolved_value: Vec<Interned> =
            value.iter()
                 .map(|v| frame.resolve(v))
                 .collect();
...
pub fn do_bind(me: &Solver,
               state: &mut RuntimeState,
               frame: &mut Frame) {
    for &(round, count) in state.output_rounds
                                .get_output_rounds()
                                .iter()
    {
        for &(e, a, v) in me.binds.iter() {
            ...
            state.distinct_index
                 .distinct(&output, &mut state.rounds);
        }
    }
}

pub fn do_commit(me: &Solver,
                 state: &mut RuntimeState,
                 frame: &mut Frame) {
    let n = (me.block * 10000) as u32;
    for &(_, count) in state.output_rounds
                            .get_output_rounds()
                            .iter()
    {
...
pub fn do_dynamic_commit(me: &Solver,
                         state: &mut RuntimeState,
                         frame: &mut Frame) {
    let n = (me.block * 10000) as u32;
    for &(_, count) in state.output_rounds
                            ...
                            .iter()
    {
        for &(e, a, v, _type) in me.dynamic_commits.iter() {
            let (correct_count, change_type) =
                if frame.resolve(&_type) == me.interned_remove {
...
pub fn do_intermediate_insert(me: &Solver,
                              state: &mut RuntimeState,
                              frame: &mut Frame) {
...
pub fn do_aggregate(me: &Solver,
    ...
    for &(ref group,
          ...
          add,
          remove,
          kind) in me.aggregates.iter()
    {
        let resolved_group: Vec<Interned> =
            group.iter()
                 .map(|v| frame.resolve(v))
                 .collect();
        ...
        let resolved_output: Vec<Interned> =
            output_key.iter()
                      .map(|v| frame.resolve(v))
                      .collect();
        for &(round, count) in state.output_rounds
