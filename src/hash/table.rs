...
/// this interface is safe, it's not used outside this module.
#[derive(Clone)]
pub struct DangerousRawBuckets<K, V> {
    raw: RawBucket<K, V>,
    elems_left: usize,
}

impl<K, V> Iterator for DangerousRawBuckets<K, V> {
    type Item = RawBucket<K, V>;
...
impl<K, V> ExactSizeIterator for DangerousRawBuckets<K, V> {
    fn len(&self) -> usize {
        self.elems_left
    }
}
...
/// Iterator over shared references to entries in a table.
#[derive(Clone)]
pub struct DangerousKeysIter<K, V> {
    iter: DangerousRawBuckets<K, V>,
}

impl<K, V> DangerousKeysIter<K, V> {
