    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            &ParseError::EmptySearch => {
                write!(f,
                       "This block has an empty search. If you want a block to run\n unconditionally, you can omit the search section.")
            }
            &ParseError::EmptyUpdate => {
                write!(f,
                       "This block doesn't have any actions in it.")
            }
            &ParseError::InvalidBlock => {
                write!(f,
                       "This block is invalid, but unfortunately I don't have a lot of information about why.")
            }
            &ParseError::MissingEnd => {
                write!(f,
                       "The `end` keyword is missing for this block.")
            }
            ...
            &ParseError::NumberOverflow() => {
                write!(f,
                       "This block contains a number too large or small to represent with the numeric datatype in use.")
            }
        }
    }
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            &Error::InvalidNeedle => {
                write!(f,
                       "The `from` in a sorted aggregate has to be the same size as the `for` in order to match the values.")
            }
            ...
            &Error::Unprovided(ref var) => {
                write!(f,
                       "Nothing in the block is providing `{}`. You can search for\n something that provides `{}`, or bind a constant.\n e.g. `{}: \"Hello\"`",
                       var,
                       var,
                       var)
            }
...
pub fn from_parse_error<'a>(error: &ParseResult<Node<'a>>)
    -> CompileError {
    match error {
