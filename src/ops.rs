...
use compiler::{FunctionKind, make_block, parse_file};
use indexes::{AggregateEntry, CollapsedChanges, DistinctIndex,
              DistinctIter, HashIndex, IntermediateIndex, MyHasher,
              RawRemoteChange, RemoteChange, RemoteIndex, WatchIndex};
...
pub fn n(num: f32) -> Internable { Internable::from_number(num) }
...
impl Block {
    pub fn new(interner: &mut Interner,
               name: &str,
               block_id: Interned,
        ...
        me.solver = Some(Solver::new(interner,
                                     block_id,
                                     0,
                                     None,
                                     &me.constraints));
    ...
    pub fn to_shapes(&self) -> Vec<Vec<PipeShape>> {
        ...
        let mut tag_mappings: HashMap<Field, Vec<Interned>> =
            HashMap::new();
        ...
        for scan in scans.iter() {
            match scan {
                ...
                &&Constraint::LookupCommit {
                    ...
                    let actual_a = if let &Field::Value(val) = a {
                        val
                    } else {
                        0
                    };
                    let actual_v = if let &Field::Value(val) = v {
                        val
                    } else {
                        0
                    };
        ...
        for scan in scans.iter() {
            ...
            match scan {
                ...
                &&Constraint::LookupCommit {
                    ...
                    let actual_e = if let &Field::Value(val) = e {
                        val
                    } else {
                        0
                    };
                    ...
                    if actual_e != 0 {
                        scan_shapes.push(PipeShape::Scan(actual_e,
                                                         0,
                                                         0));
                    } else if actual_a == tag {
                        scan_shapes.push(PipeShape::Scan(0,
                                                         actual_a,
                                                         actual_v));
                    } else {
                ...
                &&Constraint::AntiScan { ref key, .. } => {
                    if let Field::Value(id) = key[0] {
                        ...
                        panic!("Non value intremediate id: {:?}",
                               scan);
                    }
                }
...
impl Row {
    ...
    pub fn clear_solving_for(&mut self) { self.solving_for = 0; }

    pub fn clear(&mut self, field_index: usize) {
        self.fields[field_index] = 0;
        self.solving_for = 0;
        self.solved_fields = clear_bit(self.solved_fields,
                                       field_index);
    }
...
impl Field {
    ...
    pub fn to_value(&self) -> Interned {
        if let &Field::Value(v) = self { v } else { 0 }
    }
}

pub fn register(ix: usize) -> Field { Field::Register(ix) }
...
pub fn format_field(interner: &Interner, field: &Field) -> String {
    match field {
        ...
        &Field::Value(interned) => {
            format!("Value({})", format_interned(interner, interned))
        }
    }
}
...
impl PartialOrd for Internable {
    fn partial_cmp(&self, rhs: &Self) -> Option<cmp::Ordering> {
        ...
        match (self, rhs) {
            (&Internable::Null, &Internable::Null) => {
                Some(cmp::Ordering::Equal)
            }
            (&Internable::String(ref s),
             &Internable::String(ref s2)) => {
...
impl Internable {
    pub fn to_number(intern: &Internable) -> f32 {
        match intern {
            &Internable::Number(num) => unsafe {
                transmute::<u32, f32>(num)
            },
    ...
    pub fn to_json(&self) -> JSONInternable {
        match self {
            &Internable::String(ref s) => {
                JSONInternable::String(s.to_owned())
            }
...
impl JSONInternable {
    pub fn to_number(intern: &JSONInternable) -> f32 {
        match intern {
            &JSONInternable::Number(num) => unsafe {
                transmute::<u32, f32>(num)
            },
            _ => panic!("to_number on non-number"),
        }
...
impl Interner {
    ...
    pub fn internable_to_id(&mut self,
                            thing: Internable)
        -> Interned {
...
type MultiFunction = fn(Vec<&Internable>)
                        -> Option<Vec<Vec<Internable>>>;
pub type AggregateFunction = fn(&mut AggregateEntry,
                                &Vec<Internable>,
                                &Vec<Internable>);
...
fn replace_registers(fields: &mut Vec<&mut Field>,
                     lookup: &HashMap<Field, Field>) {
    for field in fields {
...
impl Constraint {
    pub fn get_registers(&self) -> Vec<Field> {
        match self {
            ...
            &Constraint::LookupRemote {
                ...
                filter_registers(&vec![e, a, v, _for, _type, from,
                                       to])
            }
            &Constraint::AntiScan { ref key, .. } => {
                filter_registers(&key.iter().collect())
            }
            &Constraint::IntermediateScan {
                ref full_key, ..
            ...
            &Constraint::RemoveAttribute { ref e, ref a } => {
                filter_registers(&vec![e, a])
            }
            &Constraint::RemoveEntity { ref e } => {
                filter_registers(&vec![e])
            }
    ...
    pub fn replace_registers(&mut self,
                             lookup: &HashMap<Field, Field>) {
        match self {
            &mut Constraint::Scan {
                ref mut e,
            ...
            &mut Constraint::LookupRemote {
                ...
                replace_registers(&mut vec![e, a, v, _for, _type,
                                            from, to],
                                  lookup);
                *register_mask =
                    make_register_mask(vec![e, a, v, _type, _for,
                                            from, to]);
            }
            &mut Constraint::AntiScan {
                ...
                replace_registers(&mut key.iter_mut().collect(),
                                  lookup);
                *register_mask = make_register_mask(key.iter()
                                                       .collect());
            }
            &mut Constraint::IntermediateScan {
                ...
                replace_registers(&mut full_key.iter_mut().collect(),
                                  lookup);
                replace_registers(&mut key.iter_mut().collect(),
                                  lookup);
                *register_mask = make_register_mask(key.iter()
                                                       .collect());
                replace_registers(&mut value.iter_mut().collect(),
                                  lookup);
                *output_mask = make_register_mask(value.iter()
                                                       .collect());
            }
            &mut Constraint::Function {
                ...
                *param_mask = make_register_mask(params.iter()
                                                       .collect());
            ...
            &mut Constraint::MultiFunction {
                ...
                *param_mask = make_register_mask(params.iter()
                                                       .collect());
                *output_mask = make_register_mask(outputs.iter()
                                                         .collect());
            }
            ...
            &mut Constraint::Project { ref mut registers } => {
                for reg in registers.iter_mut() {
                    if let &Field::Register(neue) =
                        lookup.get(&Field::Register(*reg))
                              .unwrap()
                    {
                        *reg = neue;
                    }
            ...
            &mut Constraint::Watch { ref mut registers, .. } => {
                replace_registers(&mut registers.iter_mut()
                                                .collect(),
                                  lookup);
            }
        }
...
impl Clone for Constraint {
    fn clone(&self) -> Self {
        match self {
            ...
            &Constraint::Project { ref registers } => {
                Constraint::Project { registers: registers.clone() }
            }
            &Constraint::Watch {
                ref name,
                ref registers,
...
impl PartialEq for Constraint {
    fn eq(&self, other: &Constraint) -> bool {
        match (self, other) {
            ...
            (&Constraint::RemoveEntity { e },
             &Constraint::RemoveEntity { e: e2 }) => e == e2,
            (&Constraint::DynamicCommit { e, a, v, _type },
             &Constraint::DynamicCommit {
                 e: e2,
                 a: a2,
            ...
            (&Constraint::Project { ref registers },
             &Constraint::Project { registers: ref registers2 }) => {
...
pub fn make_commit_lookup(e: Field,
                          a: Field,
                          v: Field)
    -> Constraint {
    let register_mask = make_register_mask(vec![&e, &a, &v]);
...
pub fn make_remote_lookup(e: Field,
                          a: Field,
                          v: Field,
                          _for: Field,
                          _type: Field,
                          ...
                          to: Field)
    -> Constraint {
    let register_mask = make_register_mask(vec![&e, &a, &v, &_for,
                                                &_type, &from, &to]);
    Constraint::LookupRemote {
        e,
        a,
...
pub fn make_anti_scan(key: Vec<Field>) -> Constraint {
    let register_mask =
        make_register_mask(key.iter().collect::<Vec<&Field>>());
    Constraint::AntiScan { key, register_mask }
}

pub fn make_intermediate_scan(key: Vec<Field>,
                              value: Vec<Field>)
    -> Constraint {
    ...
    let register_mask =
        make_register_mask(key.iter().collect::<Vec<&Field>>());
    let output_mask =
        make_register_mask(value.iter().collect::<Vec<&Field>>());
    Constraint::IntermediateScan {
        full_key,
        key,
        value,
...
pub fn make_intermediate_insert(key: Vec<Field>,
                                value: Vec<Field>,
                                negate: bool)
    -> Constraint {
...
pub fn make_function(op: &str,
                     params: Vec<Field>,
                     output: Field)
    -> Constraint {
    let param_mask =
        make_register_mask(params.iter().collect::<Vec<&Field>>());
...
pub fn make_multi_function(op: &str,
                           params: Vec<Field>,
                           outputs: Vec<Field>)
    -> Constraint {
    let param_mask =
        make_register_mask(params.iter().collect::<Vec<&Field>>());
    let output_mask =
        make_register_mask(outputs.iter().collect::<Vec<&Field>>());
    let func = match op {
...
pub fn make_aggregate(op: &str,
    ...
    let param_mask =
        make_register_mask(params.iter().collect::<Vec<&Field>>());
    let output_mask =
        make_register_mask(output.iter().collect::<Vec<&Field>>());
    let (add, remove): (AggregateFunction, AggregateFunction) =
        match op {
            "gather/sum" => (aggregate_sum_add, aggregate_sum_remove),
            "gather/count" => {
                (aggregate_count_add, aggregate_count_remove)
            }
            "gather/average" => {
                (aggregate_avg_add, aggregate_avg_remove)
            }
            "gather/string-join" => {
                (aggregate_string_join_add,
                 aggregate_string_join_remove)
            }
            "gather/top" => (aggregate_top_add, aggregate_top_remove),
            "gather/bottom" => {
                (aggregate_bottom_add, aggregate_bottom_remove)
            }
            "gather/next" => {
                (aggregate_next_add, aggregate_next_remove)
            }
            "gather/previous" => {
...
pub fn math_range(params: Vec<&Internable>)
    -> Option<Vec<Vec<Internable>>> {
    match params.as_slice() {
        &[&Internable::Number(_), &Internable::Number(_)] => {
...
pub fn random_number(params: Vec<&Internable>) -> Option<Internable> {
    match params.as_slice() {
        &[&Internable::Number(seed)] => {
            let mut rng = XorShiftRng::from_seed([0x123, seed,
                                                  !seed, seed]);
        ...
        &[&Internable::String(ref text)] => {
            ...
            let mut rng = XorShiftRng::from_seed([0x123,
                                                  top,
                                                  top | bottom,
                                                  bottom]);
            Some(Internable::from_number(rng.next_f32()))
...
pub fn string_replace(params: Vec<&Internable>)
    -> Option<Internable> {
    match params.as_slice() {
...
pub fn string_contains(params: Vec<&Internable>)
    -> Option<Internable> {
    match params.as_slice() {
...
pub fn string_lowercase(params: Vec<&Internable>)
    -> Option<Internable> {
    match params.as_slice() {
        &[&Internable::String(ref text)] => {
            Some(Internable::String(text.to_lowercase()))
        }
        _ => None,
    }
}

pub fn string_uppercase(params: Vec<&Internable>)
    -> Option<Internable> {
    match params.as_slice() {
        &[&Internable::String(ref text)] => {
            Some(Internable::String(text.to_uppercase()))
        }
        _ => None,
    }
}
...
pub fn string_substring(params: Vec<&Internable>)
    -> Option<Internable> {
    let params_slice = params.as_slice();
    match params_slice {
        &[&Internable::String(ref text), _..] => {
            ...
            if start > end {
                ...
                Some(Internable::String(graphemes[start..end]
                                            .join("")))
            }
        }
        _ => None,
...
pub fn string_index_of(params: Vec<&Internable>)
    -> Option<Vec<Vec<Internable>>> {
    match params.as_slice() {
        &[&Internable::String(ref text), &Internable::String(ref substring)] => {
...
pub fn string_split(params: Vec<&Internable>)
    -> Option<Vec<Vec<Internable>>> {
    match params.as_slice() {
...
pub fn string_split_reverse(params: Vec<&Internable>)
    -> Option<Vec<Vec<Internable>>> {
    match params.as_slice() {
        &[&Internable::String(ref text), &Internable::String(ref by)] => {
...
pub fn concat(params: Vec<&Internable>) -> Option<Internable> {
    ...
    for param in params {
        match param {
            ...
            &Internable::Number(_) => {
                result.push_str(&Internable::to_number(param)
                                    .to_string());
            }
            _ => {}
        }
...
pub fn eve_type_of(params: Vec<&Internable>) -> Option<Internable> {
    match params.get(0) {
        Some(&&Internable::String(_)) => {
            Some(Internable::String("string".to_owned()))
        }
        Some(&&Internable::Number(_)) => {
            Some(Internable::String("number".to_owned()))
        }
...
pub fn aggregate_sum_add(current: &mut AggregateEntry,
...
pub fn aggregate_sum_remove(current: &mut AggregateEntry,
                            params: &Vec<Internable>,
                            _: &Vec<Internable>) {
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
...
pub fn aggregate_count_add(current: &mut AggregateEntry,
                           _: &Vec<Internable>,
                           _: &Vec<Internable>) {
    match current {
        &mut AggregateEntry::Result(ref mut res) => {
...
pub fn aggregate_count_remove(current: &mut AggregateEntry,
                              _: &Vec<Internable>,
                              _: &Vec<Internable>) {
    match current {
        &mut AggregateEntry::Result(ref mut res) => {
...
pub fn aggregate_avg_add(current: &mut AggregateEntry,
                         params: &Vec<Internable>,
                         _: &Vec<Internable>) {
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
...
pub fn aggregate_avg_remove(current: &mut AggregateEntry,
                            params: &Vec<Internable>,
                            _: &Vec<Internable>) {
    match params.as_slice() {
        &[ref param @ Internable::Number(_)] => {
...
pub fn aggregate_string_join_add(current: &mut AggregateEntry,
                                 params: &Vec<Internable>,
...
pub fn aggregate_string_join_remove(current: &mut AggregateEntry,
                                    params: &Vec<Internable>,
                                    projection: &Vec<Internable>) {
    let value = params.iter().map(|x| {
        match x {
...
fn is_aggregate_in_round(&(_, v): &(&Vec<Internable>, &Vec<Count>),
                         round: Round)
    -> bool {
...
pub fn aggregate_top_add(current: &mut AggregateEntry,
                         params: &Vec<Internable>,
                         _: &Vec<Internable>) {
    ...
    } = current
    {
        if let &Some(ref limit_params) = current_params {
            ...
            if let Some(interned_limit @ &Internable::Number(_)) =
                limit_param
            {
                let limit = Internable::to_number(interned_limit) as
                    usize;
...
pub fn aggregate_top_remove(current: &mut AggregateEntry,
                            params: &Vec<Internable>,
                            _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_bottom_add(current: &mut AggregateEntry,
                            params: &Vec<Internable>,
                            _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_bottom_remove(current: &mut AggregateEntry,
                               params: &Vec<Internable>,
                               _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_next_add(current: &mut AggregateEntry,
                          params: &Vec<Internable>,
                          _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_next_remove(current: &mut AggregateEntry,
                             params: &Vec<Internable>,
                             _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_prev_add(current: &mut AggregateEntry,
                          params: &Vec<Internable>,
                          _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn aggregate_prev_remove(current: &mut AggregateEntry,
                             params: &Vec<Internable>,
                             _: &Vec<Internable>) {
    if let &mut AggregateEntry::Sorted {
        ref mut items,
...
pub fn set_bit(solved: u64, bit: usize) -> u64 { solved | (1 << bit) }
...
fn collapse_rounds(results: &Vec<RoundCount>,
                   collapsed: &mut Vec<RoundCount>) {
    collapsed.clear();
    let mut prev = (0, 0);
...
impl OutputRounds {
    ...
    pub fn get_output_rounds(&self) -> &Vec<RoundCount> {
        match (self.output_rounds.len(),
                 self.prev_output_rounds.len()) {
            (0, _) => &self.prev_output_rounds,
            (_, 0) => &self.output_rounds,
            (..) => panic!("neither round array is empty"),
        }
    }

    fn fetch_neue_current(&mut self)
        -> (&mut Vec<RoundCount>,
            &mut Vec<RoundCount>,
            &mut Vec<RoundCount>) {
        match (self.output_rounds.len(),
    ...
    pub fn compute_anti_output_rounds(&mut self,
                                      right_iter: DistinctIter) {
    ...
    pub fn compute_output_rounds(&mut self,
                                 right_iter: DistinctIter) {
        let (neue, current, result) = self.fetch_neue_current();
        result.clear();
...
pub struct RoundHolder {
    ...
    commits: HashMap<(Interned, Interned, Interned, Interned),
                     (ChangeType, Change)>,
...
impl RoundHolder {
    ...
    pub fn prepare_commits(&mut self,
                           index: &mut HashIndex,
    ...
    pub fn iter(&self) -> RoundHolderIter { RoundHolderIter::new() }
}
...
impl RoundHolderIter {
    ...
    pub fn next_round(&mut self,
        ...
        let max_round = holder.max_round;
        {
            ...
            cur_changes.clear();
            self.change_ix = 0;
            while round_ix <= max_round + 1 &&
                cur_changes.len() == 0
            {
                for (_, change) in holder.rounds[round_ix]
                    .drain()
...
pub struct BlockInfo {
    pub pipe_lookup:
        HashMap<(Interned, Interned, Interned), Vec<Solver>>,
...
impl Program {
    ...
    pub fn clear(&mut self) { self.state.index = HashIndex::new(); }
    ...
    pub fn raw_insert(&mut self,
        ...
        self.state
            .distinct_index
            .raw_insert(e, a, v, round, count);
        if count > 0 {
            self.state
                .distinct_index
                .insert_active(e, a, v, round);
            self.state.index.insert(e, a, v);
        } else {
            self.state
                .distinct_index
                .remove_active(e, a, v, round);
    ...
    pub fn register_block(&mut self, mut block: Block) {
        ...
        for (pipe, shapes) in
            pipes.drain(..).zip(block.shapes.iter())
        {
            for shape in shapes {
                match shape {
...
fn intermediate_flow(frame: &mut Frame,
                     state: &mut RuntimeState,
    ...
    *max_round = cmp::max(*max_round, intermediate_max);
    if let Some(_) = state.intermediates
                          .rounds
                          .get(&current_round)
    {
        let mut remaining: Vec<(Vec<Interned>,
                                IntermediateChange)> =
            state.intermediates
                 .rounds
        ...
        while remaining.len() > 0 {
            for (_, cur) in remaining {
                ...
                state.intermediates
                     .update_active_rounds(&cur);
                if let Some(ref actives) =
                    block_info.intermediate_pipe_lookup
                              .get(&cur.key[0])
                {
                    frame.reset();
...
fn transaction_flow(commits: &mut Vec<Change>,
                    frame: &mut Frame,
                    iter_pool: &mut EstimateIterPool,
                    program: &mut Program) {
...
fn transaction_flow_meta(commits: &mut Vec<Change>,
                         frame: &mut Frame,
                         iter_pool: &mut EstimateIterPool,
                         program: &mut Program,
                         maybe_meta: Option<&mut MetaMessage>) {
    {
        ...
        while next_frame {
            ...
            let mut max_round: Round =
                program.state.rounds.max_round as Round;
            let mut items = program.state.rounds.iter();
            while current_round <= max_round {
                let round =
                    items.get_round(&mut program.state.rounds,
                                    current_round);
                for change in round.iter() {
                    ...
                    if change.count > 0 {
                        if program.state
                                  .distinct_index
                                  .insert_active(change.e,
                                                 change.a,
                                                 change.v,
                                                 change.round)
                        {
                            let added =
                                program.state.index.insert(change.e,
                                                           change.a,
                                                           change.v);
                    ...
                    pipes.clear();
                    program.get_pipes(&program.block_info,
                                      change,
                                      &mut pipes);
                    frame.reset();
                    ...
                    if change.count < 0 {
                        if program.state
                                  .distinct_index
                                  .remove_active(change.e,
                                                 change.a,
                                                 change.v,
                                                 change.round)
                        {
                            let removed =
                                program.state.index.remove(change.e,
                                                           change.a,
                                                           change.v);
                            if let Some(&mut MetaMessage::Transaction{ref mut outputs, ..}) = maybe_meta {
                ...
                intermediate_flow(frame,
                                  &mut program.state,
                                  &program.block_info,
                                  iter_pool,
                                  current_round,
                                  &mut max_round);
                max_round = cmp::max(max_round,
                                     program.state.rounds.max_round as
                                         Round);
                current_round += 1;
            }
...
pub fn exec_meta(&mut self, program: &mut Program, persistence_channel: &mut Option<Sender<PersisterMessage>>, maybe_meta: Option<&mut MetaMessage>){
        ...
        {
            inputs.extend(self.changes.iter().map(|c| {
                c.to_raw(&program.state.interner)
            }));
        }
        ...
        transaction_flow_meta(&mut self.commits,
                              &mut self.frame,
                              self.iter_pool,
                              program,
                              maybe_meta);
        if let &mut Some(ref channel) = persistence_channel {
            ...
            for commit in self.collapsed_commits.drain() {
                to_persist.push(commit.to_raw(&program.state
                                                      .interner));
            }
...
impl<'a> RemoteTransaction<'a> {
    pub fn new(iter_pool: &mut EstimateIterPool)
        -> RemoteTransaction {
        let frame = Frame::new();
        RemoteTransaction {
            changes: vec![],
...
pub fn exec(&mut self, program: &mut Program, persistence_channel: &mut Option<Sender<PersisterMessage>>){
        ...
        for change in self.changes.drain(..) {
                       ...
                       .get(&0)
            {
                ...
                for pipe in pipes.iter() {
                    frame.row.reset();
                    pipe.run_remote(&mut program.state,
                                    iter_pool,
                                    frame);
                }
            }
        ...
        transaction_flow(&mut self.commits,
                         frame,
                         iter_pool,
                         program);
...
impl CodeTransaction {
    ...
    pub fn exec(&mut self,
        ...
        for name in to_remove {
            {
                let block_ix =
                    match program.block_info
                                   .block_names
                                   .get(&name) {
                        Some(v) => *v,
                        _ => {
                            panic!("Unable to find block to remove: '{}'",
                                   name)
                        }
                    };
    ...
    pub fn to_eve_value(&self,
        ...
        match self {
            &PortableField::Register(ix) => {
                ...
                changes.push(RawChange::new(id.clone(),
                                            s("tag"),
                                            s("register"),
                                            s("compiler"),
                                            1));
...
impl Field {
    pub fn to_portable(&self, interner: &Interner) -> PortableField {
        match self {
            ...
            &Field::Value(interned) => {
                PortableField::Value(interner.get_value(interned)
                                             .clone())
            }
        }
    }
...
impl PortableConstraint {
    pub fn intern(&self, interner: &mut Interner) -> Constraint {
        match self {
            &PortableConstraint::Scan(ref e, ref a, ref v) => {
                make_scan(e.intern(interner),
                          a.intern(interner),
                          v.intern(interner))
            }
            &PortableConstraint::Output(ref e,
                                        ref a,
                                        ref v,
                                        commit) => {
                Constraint::Insert {
            ...
            &PortableConstraint::Watch(ref name, ref registers) => {
                Constraint::Watch {
                    ...
                    registers: registers.iter()
                                        .map(|v| v.intern(interner))
                                        .collect(),
                }
            }
            &PortableConstraint::Function(ref name,
                                          ref output,
                                          ref args) => {
    ...
    pub fn to_raw_changes(&self,
        ...
        match self {
            ...
            &PortableConstraint::Filter(ref op,
                                        ref left,
                                        ref right) => {
            ...
            &PortableConstraint::Function(ref name,
                ...
                for (ix, raw_arg) in args.iter().enumerate() {
                    ...
                    changes.push(RawChange::new(arg_id.clone(),
                                                s("value"),
                                                arg,
                                                s("compiler"),
                                                1));
            ...
            &PortableConstraint::MultiFunction(ref name,
                                               ref outputs,
                                               ref args) => {
                changes.push(RawChange::new(id.clone(),
                                            s("tag"),
                ...
                for (ix, raw_output) in outputs.iter().enumerate() {
                    let output =
                        raw_output.to_eve_value(block, changes);
                    let eve_ix = n((ix + 1) as f32);
                    let output_id = gen_id(vec![&eve_ix, &output])
                        .unwrap();
                    changes.push(RawChange::new(output_id.clone(),
                                                s("value"),
                                                output,
                                                s("compiler"),
                                                1));
    ...
    pub fn to_portable(&self, i: &Interner) -> PortableConstraint {
        match self {
            &Constraint::Scan {
                ...
                PortableConstraint::Scan(e.to_portable(i),
                                         a.to_portable(i),
                                         v.to_portable(i))
            }
            &Constraint::Filter {
                ...
                PortableConstraint::Filter(op.to_owned(),
                                           left.to_portable(i),
                                           right.to_portable(i))
            }
            &Constraint::Insert {
                ...
                PortableConstraint::Output(e.to_portable(i),
                                           a.to_portable(i),
                                           v.to_portable(i),
                                           commit)
            }
            &Constraint::Remove {
                ...
                PortableConstraint::Remove(e.to_portable(i),
                                           a.to_portable(i),
                                           v.to_portable(i))
            }
            &Constraint::RemoveAttribute { ref e, ref a } => {
                PortableConstraint::RemoveAttribute(e.to_portable(i),
                                                    a.to_portable(i))
            }
            &Constraint::RemoveEntity { ref e } => {
...
impl PortableBlock {
    pub fn intern(&self, interner: &mut Interner) -> Block {
        let constraints = self.constraints
                              ...
                              .collect();
        let block_id =
            interner.internable_to_id(self.block_id.clone());
        Block::new(interner, &self.name, block_id, constraints)
    }

    pub fn to_raw_changes(&self, changes: &mut Vec<RawChange>) {
        let id_str = format!("block|{}",
                             Internable::to_string(&self.block_id));
        let id = s(&id_str);
        changes.push(RawChange::new(id.clone(),
                                    s("tag"),
                                    s("block"),
        ...
        for (ix, constraint) in self.constraints.iter().enumerate() {
            let constraint_id =
                constraint.to_raw_changes(&id, ix, changes);
            changes.push(RawChange::new(id.clone(),
                                        s("constraint"),
                                        constraint_id,
                                        s("compiler"),
...
impl Block {
    pub fn to_portable(&self, interner: &Interner) -> PortableBlock {
        ...
        PortableBlock {
            ...
            block_id: interner.get_value(self.block_id)
                              .clone(),
            constraints,
        }
    }
...
impl Persister {
    ...
    pub fn wait(self) { self.thread.join().unwrap(); }
