...
use super::super::ops::{Internable, Interner, RawChange,
                        RunLoopMessage};
use std::fs::File;
use std::io::Error;
use std::io::prelude::*;
...
impl Watcher for FileWatcher {
    ...
    fn on_diff(&mut self, interner: &mut Interner, diff: WatchDiff) {
        for add in diff.adds {
            let kind =
                Internable::to_string(interner.get_value(add[0]));
            let record_id =
                Internable::to_string(interner.get_value(add[1]));
            let id = Internable::String(format!("file/{}/change/{}",
                                                kind,
                                                record_id));
            let raw_path =
                Internable::to_string(interner.get_value(add[2]));
            ...
            match &kind[..] {
                "read" => {
                    match File::open(&path) {
                        Err(why) => {
                            file_error(&mut changes, record_id, why)
                        }
                        Ok(mut file) => {
                            let mut contents = String::new();
                            match file.read_to_string(&mut contents) {
                                Err(why) => {
                                    file_error(&mut changes,
                                               record_id,
                                               why)
                                }
                                Ok(_) => {
