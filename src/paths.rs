...
impl<'a> EvePaths<'a> {
    ...
    pub fn persist(&self) -> Option<&str> {
        self.persist_path.as_ref().and_then(|ref path| path.to_str())
    }
    pub fn libraries(&self) -> Option<&str> {
        self.libraries_path.as_ref().and_then(|ref path| path.to_str())
    }
    pub fn programs(&self) -> Option<&str> {
        self.programs_path.as_ref().and_then(|ref path| path.to_str())
    }
}
