    ...
    for university_ix in 0..university_count {
        ...
        for department_ix in 0..department_count {
            ...
            for g_ix in 0..grads_count {
                ...
                // every GraduateStudent co-authors 0~5 Publications with some Professors
                // every GraudateStudent has an undergraduateDegreeFrom a University
                let degree = rand_between(&mut seed, 0, 500);//university_count as u32);
...
pub mod tests {
    ...
    pub fn exec_query(program:&mut Program, constraints: Vec<Constraint>) {
        ...
        println!("Run took {:?}", (end_ns - start_ns) as f64 / 10_000_000.0);
    ...
    #[bench]
    pub fn bench_lubm(b: &mut Bencher) {
        let mut program = Program::new();
        setup(&mut program, 1);

        let constraints = lubm_2(&mut program);
    ...
    pub fn lubm_3(program:&mut Program) -> Vec<Constraint> {
        vec![
            ...
            make_scan(register(0), program.interner.string("author"), program.interner.string("univeristy0|department0|assistant_professor0")),
            Constraint::Project { registers: vec![0] },
        ]
    }
    ...
    pub fn lubm_5(program:&mut Program) -> Vec<Constraint> {
        vec![
            ...
            make_scan(register(0), program.interner.string("member of"), program.interner.string("univeristy0|department0")),
            Constraint::Project { registers: vec![0] },
        ]
    }
    ...
    pub fn lubm_7(program:&mut Program) -> Vec<Constraint> {
        vec![
            make_scan(register(0), program.interner.string("tag"), program.interner.string("undergraduat student")),
            ...
            make_scan(program.interner.string("university0|department0|associate_professor0"), program.interner.string("teacher of"), register(2)),
            make_scan(register(0), program.interner.string("telephone"), register(3)),
            Constraint::Project { registers: vec![0, 1, 2, 3] },
        ]
    }
    ...
    pub fn lubm_11(program:&mut Program) -> Vec<Constraint> {
        vec![
            ...
            make_scan(register(0), program.interner.string("suborganization of"), program.interner.string("univeristy0")),
            Constraint::Project { registers: vec![0] },
        ]
    }
    ...
    pub fn lubm_13(program:&mut Program) -> Vec<Constraint> {
        vec![
            make_scan(register(0), program.interner.string("tag"), program.interner.string("graduate student")),
            make_scan(register(0), program.interner.string("undergraduate degree from"), program.interner.string("univeristy5")),
            Constraint::Project { registers: vec![0] },
        ]
    }
